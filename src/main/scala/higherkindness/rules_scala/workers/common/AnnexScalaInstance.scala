...
final class AnnexScalaInstance(val allJars: Array[File]) extends ScalaInstance {
  ...
  override def compilerJar: File = allJars(0)
  ...
  override lazy val loader: ClassLoader =
    new URLClassLoader(allJars.map(_.toURI.toURL), ClassLoader.getPlatformClassLoader())
...
//      if (actualVersion.startsWith("3")) classOf[CompilerInterface2].getClassLoader
//      else ClassLoader.getPlatformClassLoader()
//    )
//
